...
void IjkGridParametricRepresentation::getParametersOfNodes(double * parameters, bool reverseIAxis, bool reverseJAxis, bool reverseKAxis) const
{
	...
	if (reverseIAxis || reverseJAxis || reverseKAxis)
	{
		const uint64_t iCellCount = getICellCount();
		const uint64_t jCellCount = getJCellCount();
		const uint64_t kCellCount = getKCellCount();
		...
		const uint64_t splitCoordinateLineCount = getSplitCoordinateLineCount();
	...
	unsigned int const * splitCoordinateLineColumnCumulativeCount, unsigned int const * splitCoordinateLineColumns, EML2_NS::AbstractHdfProxy * proxy)
{
	...
	if (splitCoordinateLineCount == 0) {
		...
		// PointParameters
		uint64_t numValues[2] = { kNodeCountPluskGapCount, jNodeCount * iNodeCount + splitCoordinateLineCount };
	...
	// *********************************
	// HDF control points
	uint64_t controlPointCount[4] = { controlPointCountPerPillar, jNodeCount, iNodeCount, 3 };
