...
#ifdef __cplusplus
SOAP_SOURCE_STAMP("@(#) stdsoap2.cpp ver 2.8.111E 2021-01-22 00:00:00 GMT")
extern "C" {
#else
SOAP_SOURCE_STAMP("@(#) stdsoap2.c ver 2.8.111E 2021-01-22 00:00:00 GMT")
#endif
...
http_post(struct soap *soap, const char *endpoint, const char *host, int port, const char *path, const char *action, ULONG64 count)
{
  ...
  if (err)
    return err;
  err = soap->fposthdr(soap, "User-Agent", "gSOAP/2.8E");
  if (err)
    return err;
  if (soap->origin)
  {
...
#endif
  err = soap->fposthdr(soap, "Server", "gSOAP/2.8E");
  if (err)
    return err;
  if (soap->cors_origin)
  {
