		...
		const std::string & title):resqmlWellboreFrameRepresentation(nullptr)
{
	...
	gsoapProxy2_1 = soap_new_witsml2__Log(witsmlWellbore->getGsoapContext(), 1);
...
gsoap_eml2_1::eml21__DataObjectReference* Log::getWellboreDor() const
{
	return static_cast<witsml2__Log*>(gsoapProxy2_1)->Wellbore;
}
...
void Log::setWellbore(Wellbore* witsmlWellbore)
{
	...
	// XML
	if (updateXml) {
		witsml2__Log* log = static_cast<witsml2__Log*>(gsoapProxy2_1);
		log->Wellbore = witsmlWellbore->newEmlReference();
	}
}
