	...
	const double & mdTopSample)
{
	...
	gsoapProxy2_1 = soap_new_witsml2__WellboreMarkerSet(witsmlWellbore->getGsoapContext(), 1);
...
gsoap_eml2_1::eml21__DataObjectReference* WellboreMarkerSet::getWellboreDor() const
{
	return static_cast<witsml2__WellboreMarkerSet*>(gsoapProxy2_1)->Wellbore;
}
...
void WellboreMarkerSet::setWellbore(Wellbore* witsmlWellbore)
{
	...
	// XML
	if (updateXml) {
		witsml2__WellboreMarkerSet* wms = static_cast<witsml2__WellboreMarkerSet*>(gsoapProxy2_1);
		wms->Wellbore = witsmlWellbore->newEmlReference();
	}
}
