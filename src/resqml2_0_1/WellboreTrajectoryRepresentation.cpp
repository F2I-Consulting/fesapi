...
void WellboreTrajectoryRepresentation::setWitsmlTrajectory(WITSML2_1_NS::Trajectory * witsmlTraj)
{
...
void WellboreTrajectoryRepresentation::importRelationshipSetFromEpc(COMMON_NS::EpcDocument* epcDoc)
{
	...
	if (rep->WitsmlTrajectory != nullptr) {
		WITSML2_1_NS::Trajectory* tmp = static_cast<WITSML2_1_NS::Trajectory*>(epcDoc->getResqmlAbstractObjectByUuid(rep->WitsmlTrajectory->UUID));
		if (tmp)
		{
			updateXml = false;
