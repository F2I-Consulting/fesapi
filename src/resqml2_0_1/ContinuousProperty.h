...
{
	/**
	 * Most common type of property used for storing rock or fluid attributes; all are represented
	 * as floating point values. So that the value range can be known before accessing all values,
	 * the min and max values of the range are also optionally stored. BUSINESS RULE: It also
	 * contains a unit of measure, which can be different from the unit of measure of its property
	 * type, but it must be convertible into this unit.
	 */
	...
	public:
		...
		/**
		 * Creates an instance of this class in a gsoap context based on a standard kind and a standard
		 * uom.
		 *
		 ...
		 * @param 		  	dimension			   	The dimension of each value (scalar properties == 1).
		 * @param 		  	attachmentKind		   	The topological orbit which supports each value.
		 * @param 		  	uom					   	The uom of the values. Please check
		 * 											EpcDocument::getEnergisticsUnitOfMeasure(const
		 * 											std::string &amp; energisticsUomName) in order to
		 * 											minimize the use of non standard uom.
		 ...
		 * @param 		  	attachmentKind	The topological orbit which supports each value.
		 * @param 		  	uom			  	The uom of the values. Please check
		 * 									EpcDocument::getEnergisticsUnitOfMeasure(const std::string
		 * 									&amp; energisticsUomName) in order to minimize the use of non
		 * 									standard uom.
		 ...
		 * @param 		  	dimension			   	The dimension of each value (scalar properties == 1).
		 * @param 		  	attachmentKind		   	The topological orbit which supports each value.
		 * @param 		  	nonStandardUom		   	The non standard uom of the values. Please check
		 * 											EpcDocument::getEnergisticsUnitOfMeasure(const
		 * 											std::string &amp; energisticsUomName) in order to
		 * 											minimize the use of non standard uom.
		 * @param 		  	energisticsPropertyKind	The property kind of these property values which must
		...
		/**
		 * Get the unit of measure of the values of this property. If 'Euc'  is returned, you should
		 * check if an extrametadata called "Uom" also exists. If so, it would mean that the property
		 * uses a non standard uom. This is an official workaround for a known issue of Resqml 2.0.1.
		 *
		 * @returns	The uom.
		 */
		...
		/**
		 * Get the unit of measure of the values of this property as a string. If 'Euc'  is returned,
		 ...
		 * 									method.
		 * @param 		  	minimumValue	(Optional) The minimum value of the values to add. If NAN is
		 * 									provided then the minimum value will be computed from the values.
		 * @param 		  	maximumValue	(Optional) The maximum value of the values to add. If NAN is
		 * 									provided then the maximum value will be computed from the values.
		 */
		 ...
		 * @param [in,out]	minimumValue	  	(Optional) The minimum value (or value vector) of the
		 * 										values to add. If nullptr is provided and the dimension
		 * 										of value is 1 then the minimum value will be computed
		 * 										from the values.
		 * @param [in,out]	maximumValue	  	(Optional) The maximum value (or value vector) of the
		 * 										values to add. If nullptr is provided and the dimension
		 * 										of value is 1 then the maximum value will be computed
		 * 										from the values.
		 */
		 ...
		 * @param 		  	values				  	All the property values to set ordered according the
		 * 											topology of the representation it is based on.
		 * @param 		  	valueCountInFastestDim	The number of values to write in the slowest
		 * 											dimension (mainly I dimension).
		 * @param 		  	valueCountInSlowestDim	The number of values to write in the slowest
		 * 											dimension (mainly J dimension).
		 * @param [in,out]	proxy				  	(Optional) The HDF proxy where to write the property
		 ...
		 * @param 		  	valueCountInFastestDim	The number of values to write in the slowest
		 * 											dimension (mainly I dimension).
		 * @param 		  	valueCountInMiddleDim 	The number of values to write in the slowest
		 * 											dimension (mainly J dimension).
		 * @param 		  	valueCountInSlowestDim	The number of values to write in the slowest
		 * 											dimension (mainly K dimension).
		 * @param [in,out]	proxy				  	(Optional) The HDF proxy where to write the property
		...
		/**
		 * Create an array (potentially multi dimensions) of explicit float values to the property
		 * values. No values are written to this array yet then the HDF5 array contains uninitialized
		 * values.
		 *
		 * @param 		  	numValues		  	The number of property values ordered by dimension of the
		 * 										array to write.
		...
		/**
		 * Create a 3d array of explicit float values to the property values. No values are written to
		 ...
		 * 											values. It must be already opened for writing and
		 * 											won't be closed in this method.
		 * @param 		  	patchIndex			  	(Optional) The index of the patch we want to set some
		 * 											values. If not present, the last patch is arbitrarily
		 * 											taken into account.
		 */
		 ...
		 *
		 * @param [in,out]	proxy	   	The HDF5 proxy where the values are already or will be stored.
		...
		/**
		 * Get some of the values of a particular patch of the instance which are supposed to be float
		 ...
		 * 											(mainly K dimension).
		 * @param 		  	offsetInFastestDim	  	The offset value to read in the fastest dimension
		 * 											(mainly I dimension).
		 * @param 		  	offsetInMiddleDim	  	The offset value to read in the middle dimension
		 * 											(mainly J dimension).
		 * @param 		  	offsetInSlowestDim	  	The offset value to read in the slowest dimension
		 * 											(mainly K dimension).
		 */
		 ...
		 *
		 * @returns	the minimum value if present in the file otherwise NaN.
		 */
		 ...
		 *
		 * @returns	the maximum value if present in the file otherwise NaN.
		 */
