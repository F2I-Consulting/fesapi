...
namespace WITSML2_1_NS
{
	...
	public:
		...
		uint64_t getChannelIndexCount() const {
			return static_cast<T*>(this->gsoapProxy2_3)->Index.size();
		}
		...
		std::string getChannelIndexUom(uint64_t index) {
			return static_cast<T*>(this->gsoapProxy2_3)->Index.at(index)->Uom;
		}
		...
		std::string getChannelIndexMnemonic(uint64_t index) {
			return static_cast<T*>(this->gsoapProxy2_3)->Index.at(index)->Mnemonic;
		}
	};
}
