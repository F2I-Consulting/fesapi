...
std::vector<int64_t> ColumnBasedTable::getInt64Values(uint64_t columnIndex) const
{
	...
	if (intArray != nullptr) {
		...
		readArrayNdOfIntegerValues(intArray, result.data());
	}
...
void ColumnBasedTable::setInt64Values(uint64_t columnIndex, int64_t const* values, uint64_t valueCount, EML2_NS::AbstractHdfProxy* proxy)
{
	if (proxy == nullptr) {
		proxy = getRepository()->getDefaultHdfProxy();
		if (proxy == nullptr) {
	...
	// Column datatype
	int64_t n = 1; // Endianess check https://stackoverflow.com/questions/4181951/how-to-check-whether-a-system-is-big-endian-or-little-endian
	externalArray->ArrayIntegerType = (*(char*)&n == 1) ? eml23__IntegerType::arrayOfInt64LE : eml23__IntegerType::arrayOfInt64BE;
	...
	// HDF
	proxy->writeArrayNdOfInt64Values(getHdfGroup(),
		"column" + std::to_string(columnIndex),
		values, &valueCount, 1);
...
{
	if (propKind == nullptr) {
		throw std::invalid_argument("The property kind cannot be null.");
	}
