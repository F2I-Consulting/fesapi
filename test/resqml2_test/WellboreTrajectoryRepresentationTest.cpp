...
void WellboreTrajectoryRepresentationTest::initRepoHandler() {
	...
	double inclinations[4] = { 0, M_PI / 2, M_PI / 2, M_PI / 4 };
	double azimuths[4] = { 0, M_PI / 2, 0, M_PI / 2 };
	...
	double inclinations2[4] = { 0, M_PI / 2, M_PI / 2, 3 * M_PI / 4 };
	double azimuths2[4] = { 0, -M_PI / 2, 0, M_PI / 2 };
...
void WellboreTrajectoryRepresentationTest::readRepoHandler() {
	...
	REQUIRE(inclinations[0] < epsilon);
	REQUIRE(inclinations[1] > M_PI / 2 - epsilon);
	REQUIRE(inclinations[1] < M_PI / 2 + epsilon);
	REQUIRE(inclinations[2] > M_PI / 2 - epsilon);
	REQUIRE(inclinations[2] < M_PI / 2 + epsilon);
	REQUIRE(inclinations[3] > M_PI / 4 - epsilon);
	REQUIRE(inclinations[3] < M_PI / 4 + epsilon);
	REQUIRE(azimuths[0] > -epsilon);
	REQUIRE(azimuths[0] < epsilon);
	REQUIRE(azimuths[1] > M_PI / 2 - epsilon);
	REQUIRE(azimuths[1] < M_PI / 2 + epsilon);
	REQUIRE(azimuths[2] > -epsilon);
	REQUIRE(azimuths[2] < epsilon);
	REQUIRE(azimuths[3] > M_PI / 2 - epsilon);
	REQUIRE(azimuths[3] < M_PI / 2 + epsilon);
	...
	REQUIRE(inclinations[2] > M_PI / 2 - epsilon);
	REQUIRE(inclinations[2] < M_PI / 2 + epsilon);
	REQUIRE(inclinations[3] > 3*M_PI / 4 - epsilon);
	REQUIRE(inclinations[3] < 3*M_PI / 4 + epsilon);
	REQUIRE(azimuths[0] > -epsilon);
	REQUIRE(azimuths[0] < epsilon);
	REQUIRE(azimuths[1] > -M_PI / 2 - epsilon);
